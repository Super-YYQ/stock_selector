...
| `tdx_hosts` | `[]` | TDX 行情节点列表，每项为 `ip:port`。留空使用内置的国泰君安节点（`117.34.114.14/16/17/18/20/27:7709`）。每次建立连接后会先请求一根上证指数日线做健康探测，连得上但返回不了 K 线的节点会被自动跳过。2026-09-10 起通达信官方主站不再为本程序使用的协议返回 K 线，若内置节点也失效，可在此填入其他可用节点。 |
