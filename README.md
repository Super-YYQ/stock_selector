...
发布脚本不会把 `site/` 中其他未提交的页面、资源或历史文件一并带入，也不会从功能分支把 `HEAD` 直接推到 `main`。网页模板和静态资源的修改应随正常 PR 合并；自动发布只负责生成的报告数据。若 `src/`、`config/`、`scripts/`、`web/` 或运行依赖存在未提交修改，发布会停止，避免把未经确认的代码或配置所生成的结果推到线上。若 `git push` 失败，带发布标记的本地提交会保留，下一次执行会在校验内容和快进关系后优先重试。
