...
1. 更新数据并生成报告
2. 校验磁盘 `site/`（正式报告、索引一致、文件白名单）
3. 将 `site/` 全量提交到独立的 `gh-pages` 分支（含历史回填与保留期裁剪）
4. 由 GitHub Actions 部署 Pages

发布与 `main` 完全解耦：`main` 只跟踪源码与 `web/` 模板，本地未提交或未推送的开发提交**不会阻断发布**。网页模板和静态资源的修改仍应通过正常 PR 合并到 `main`；自动发布只搬运 `site/` 生成的报告数据。已发布的历史报告不会被静默改写（当日盘中快照升级为收盘正式报告除外）。若 `git push` 失败，下一次运行会重新对比远端状态后自然重试，无需人工清理。

网页顶部的日期下拉可切换查看保留期内（默认 90 天）的任一历史报告。
