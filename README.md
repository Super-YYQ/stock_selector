...
- 板块评分：按报告日解析申万行业历史分类；缺失时明确降级为市场板块，不冒充行业信号
- 个股背景：核心概念、细分行业、涨停量价线索和行业近半年阶段表现
- 股票池范围：可排除北交所、科创板、创业板或其他不关注的市场板块
- 多因子排名：板块、股性、量价、RPS、市场修正与风险扣分
- 稳健校准：截面百分位混合、宽泛策略选择性折减、最低置信分和行业分散上限
- 因子监控：每日记录核心因子覆盖率、分位数、零值率和分布漂移诊断
- 十一种策略：突破、趋势、回踩、事件、板块共振五类策略
- 单策略筛选：内置策略和安全 YAML 公式统一展示，可逐项查看独立命中股票
- 结果追踪：自动记录入选股票，以次日可成交开盘价回填扣除成本后的 1 / 3 / 5 / 10 日净收益和基准超额收益
- 双报告：格式化 Excel + 响应式网页
...
市场概览的 Top 10、观察名单和单策略筛选三个名单页各提供「识股图片」「识股文本」按钮，针对**当前可见（含搜索 / 板块过滤）的股票**生成同花顺可识别的输出：

- **识股图片**：用 Canvas 专门绘制，白底黑字、只含股票名称与 6 位代码，每页不超过 12 只，超出自动分页，可直接导入同花顺「AI 高精度识别」。它不是页面截图，不带深色背景、评分、涨跌幅或按钮等噪声。
- **识股文本**：纯文本「名称 代码」一行一只，一键复制，适合粘贴到同花顺「AI 文本识股」。

导出完全在浏览器端完成，不依赖后端服务，手机端也可长按图片保存。
...
1. 更新数据并生成报告
2. 校验当前位于与 `origin/main` 同步的 `main` 分支
3. 只提交 `latest.json`、历史索引、当日报告及历史保留产生的删除
4. 以快进方式推送到 `main`
5. 由 GitHub Actions 部署 Pages

发布脚本不会把 `site/` 中其他未提交的页面、资源或历史文件一并带入，也不会从功能分支把 `HEAD` 直接推到 `main`。网页模板和静态资源的修改应随正常 PR 合并；自动发布只负责生成的报告数据。本地未提交的 `src/`、`config/`、`scripts/`、`web/` 或运行依赖**不会阻断发布**——发布提交只含 `site/data/*` 报告文件，开发态实验（如临时勾选科创板过滤条件）不会进入该提交。发布按磁盘当前代码与配置生成报告；如需避免实验性结果上线，请在发布前提交或还原相关配置。若 `git push` 失败，带发布标记的本地提交会保留，下一次执行会在校验内容和快进关系后优先重试。
...
# 只检查将要发布的文件，不提交或推送
.\.venv\Scripts\python.exe scripts\publish_pages.py --dry-run
```

如果脚本提示本地 `main` 落后或与远端分叉，请先处理 Git 状态；落后时执行 `git pull --ff-only` 后重新生成报告，脚本不会自动合并或执行非快进推送。
...
不指定日期的自动任务会在周末或确认休市时记录为“已跳过”，不会拿上一交易日的数据重新发布。显式使用 `--date` 重跑时，该日期必须具备达到覆盖要求的股票行情和三大指数数据；缺失时任务会失败，而不是用旧行情生成错误日期的报告。
